...
async function ensureOrderPaymentStatus(db) {
    const existing = await existingColumns(db, "orders", ["payment_status"]);
    if (existing.has("payment_status")) return [];
    await db.execute(
        "ALTER TABLE `orders` ADD COLUMN `payment_status` varchar(20) NOT NULL DEFAULT 'paid' AFTER `payment_method`"
    );
...
async function ensurePaymentSchema(db) {
    ...
    return { orderColumns: await ensureOrderPaymentStatus(db) };
}
